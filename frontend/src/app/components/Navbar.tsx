    ...
    <header className="sticky top-0 z-40 border-b bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <button
          ...
          type="button"
        >
          <span className="flex size-10 items-center justify-center rounded-md bg-[#00346f] text-white">
            <Wrench className="size-5" />
          </span>
          <span>
            <span className="block text-lg font-semibold text-[#00346f]">
              OneStopShop
            </span>
          ...
          <nav className="hidden items-center gap-1 md:flex">
            {links.map((link) => (
              <Button
                className={cn(
                  "text-slate-700",
                  location.pathname === link.path && "text-[#00346f]",
                )}
                key={link.path}
